 ...
 */
namespace Blockchain\Api\Explorer;

use \Blockchain\Api\Conversion\Conversion;
/**
...
class Address 
{
    ...
    public function __construct($json) {
        ...
        if(array_key_exists('total_received', $json))
            $this->total_received = Conversion::BTC_int2str($json['total_received']);
        if(array_key_exists('total_sent', $json))
            $this->total_sent = Conversion::BTC_int2str($json['total_sent']);
        if(array_key_exists('final_balance', $json))
            $this->final_balance = Conversion::BTC_int2str($json['final_balance']);
